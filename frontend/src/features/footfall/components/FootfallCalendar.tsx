...
interface FootfallCalendarProps {
  entries: FootfallEntry[]
  isLoading: boolean
  canManage: boolean
  month: Date
  onMonthChange: (month: Date) => void
  onDayClick: (date: string, existingEntry?: FootfallEntry) => void
...
export default function FootfallCalendar({
  entries,
  isLoading,
  canManage,
  month,
  ...
  const avgFootfall = daysWithData > 0 ? Math.round(totalFootfall / daysWithData) : 0
        ...
        <CardHeader className="pb-3">
          ...
          {!isLoading && entries.length > 0 && (
            <div className="flex gap-6 text-sm text-muted-foreground mt-1">
              <span>Total: <span className="font-medium text-foreground">{totalFootfall.toLocaleString()}</span></span>
              <span>Avg/day: <span className="font-medium text-foreground">{avgFootfall.toLocaleString()}</span></span>
              <span>{daysWithData} of {daysInMonth} days logged</span>
            </div>
          )}
        </CardHeader>
        <CardContent className="pt-0">
            ...
            <div className="border border-border rounded-lg overflow-hidden">
              ...
              <div className="grid grid-cols-7">
                {cells.map((cell, i) => {
                  ...
                  const totalCount = dayEntries.reduce((s, e) => s + e.count, 0)
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (canManage) {
                          // Cell background always opens add dialog
                      ...
                      {hasData ? (
                        <div className="space-y-0.5">
                          {isSingleLocation ? (
                            ...
                            >
                              <span className="text-lg font-bold text-foreground tabular-nums">{totalCount.toLocaleString()}</span>
                            </div>
                          ) : (
                            dayEntries.slice(0, 3).map((entry) => (
                              ...
                              >
                                ...
                                <span className="font-semibold text-foreground tabular-nums">{entry.count.toLocaleString()}</span>
                              </div>
                            ))
                          )}
          ...
          {!isLoading && entries.length === 0 && (
            <div className="flex flex-col items-center justify-center text-center py-8">
              ...
              <p className="text-sm font-medium text-foreground mb-1">No entries for {monthLabel}</p>
              <p className="text-sm text-muted-foreground">
                {canManage ? 'Click on any day to add a footfall count.' : 'No footfall data recorded for this month.'}
              </p>
            </div>
          )}
        </CardContent>
